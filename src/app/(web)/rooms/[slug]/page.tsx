...
const RoomDetails = (props: { params: { slug: string } }) => {
  ...
  const { data: room, error, isLoading }: any = useSWR('/api/room/detail', fetchRoom);
  ...
  const handleBookNowClick = async () => {
    ...
    const hotelRoomSlug = room.slug;

    const stripe = await getStripe();

    try {
      const { data: stripeSession } = await axios.post('/api/stripe/rooms', {
        checkinDate,
        checkoutDate,
        adults,
          ...
          <div className='md:col-span-8 md:w-full'>
            <div>
              ...
              <div className='flex my-11'>
                {room?.offeredAmenities?.map((amenity: any, ind: number) => (
                  <div
                    key={ind}
